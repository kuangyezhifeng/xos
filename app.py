...
def bind_device():
    ...
    if request.method == 'POST':
        ...
        source_text = request.form.get('source_ips')
        dest_text = request.form.get('dest_ips')

        result = xray_device_route_handler(
            proxys,
            source_text,
            dest_text
        )
        ...
        if result.get('success'):
            ...
            # 重新生成（保证一致性）
            source_ips = proxys.device_ip.split(",") if proxys.device_ip else []
            dest_ips = proxys.dest_ip.split(",") if proxys.dest_ip else []

            if source_ips:
                xray_route_rule(
                    generate_device_route(source_ips, tag, "source"),
                    "source"
                )

            if dest_ips:
                xray_route_rule(
                    generate_device_route(dest_ips, tag, "ip"),
                    "ip"
                )

            return redirect(url_for('dashboard'))

        return redirect(url_for('bind_device', id=id))

    # ===== 3. GET：回显（关键修复点）=====
    source_ips = proxys.device_ip or ''
    dest_ips = proxys.dest_ip or ''
